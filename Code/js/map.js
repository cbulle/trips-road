...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function updateRouteSegment(index, mode, options = {}) {
        ...
        console.log(`URL appel�e (${mode}) : ${url}`);
        ...
        try {
            ...
            if (data.code !== 'Ok') {
                ...
                if (data.message && data.message.includes('Exclude flag')) {
                    alert("Ce serveur ne supporte pas cette combinaison d'options (p�ages/autoroutes) pour ce trajet.");
                } else {
                    alert(`Impossible de calculer le trajet. V�rifiez que la distance n'est pas trop grande pour le mode ${mode}.`);
                }
                return;
            }
            ...
            console.log(`-> R�sultat : ${seg.distance}km, ${seg.duration}min`);
